...
The hacking mini-game from "Fallout 3". Find out which 7-letter 
word is the passcode by using clues each guess gives you.
...
def main():
	...
	print('''Hacking Mini game, by Al Sweigart al@inventwithputhon.com. Find the password in the computer's memory You are given clues after each guess. For evample, of the secret passworkd is MONITER but the player guessed CONTAIN, they are given the hint that 2 out of 7 letters were correct, because both MONITER and CONTAIN have the letter O and N as their 2nd and 3rd letter. You get 4 guesses.\n''')
	input('Press Enter to begin...')
	
	gameWords = getWords()
	print (gameWords)
	# The "computer memory" is just a cosmetic, but it looks cool:
...
def getWords():
	...
	To make the game fair, we try to ensure that there are words with a range 	of matching numbers of letters as the secret word."""
	secretPassword = random.choice(WORDS)
	words = [secretPassword]


	# Find 2 more words; these have 0 matching letters.
	...
	# Find at least seven words that have at least on matching letter
...
def numMatchingLetters(word1, word2):
	"""Returns the number of matching letters in these 2 words."""
	matches = 0
...
def getComputerMemoryString(words):
	"""Return a string representing the "comuter memory"."""
	
	# Pick one line per word to contain a word. There are 16 lines,but
	# they are split in halves.
	linesWithWords = random.sample(range(16 * 2), len(words))		# The starting memory address (this is also cosmetic).
	...
	# Each string in the computerMemory is joined into one large
	# string to return:
	return '\n'.join(computerMemory)
